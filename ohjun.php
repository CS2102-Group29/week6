...
<body>
  ...
  <?php
    ...
    $db     = pg_connect("host=localhost port=5432 dbname=Project1 user=postgres password=qwer1234");
    ...
    if (isset($_POST['new'])) {	// Submit the update SQL command
        $result = pg_query($db, "INSERT INTO book (book_id, name, price, date_of_publication)
            VALUES ('$_POST[bookid_updated]', '$_POST[book_name_updated]', '$_POST[price_updated]', '$_POST[dop_updated]')
            ON CONFLICT (book_id) DO UPDATE
            SET name = excluded.name, price = excluded.price, date_of_publication = excluded.date_of_publication");
        if (!$result) {
            echo "Update failed!!";
        } else {
